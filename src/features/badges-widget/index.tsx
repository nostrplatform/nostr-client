  ...
  const handleCreateDefinition = useCallback(async () => {
    ...
    try {
      ...
      if (publishedRelays.size > 0) {
        ...
        // Manually add the new definition to the state for immediate feedback
        const newDef = parseBadgeDefinition(event);
        if (newDef) {
          setDefinitionEvents(prev => [...prev, event]);
        }
      } else {
  ...
  const handleAwardBadge = useCallback(async () => {
    ...
    try {
      ...
      if (publishedRelays.size > 0) {
        ...
        // Optionally trigger a re-fetch or wait for subscription update
        // Manually add the new award to the state for immediate feedback
        setAwardedEvents(prev => [...prev, event]);
      } else {
